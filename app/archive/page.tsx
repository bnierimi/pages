...
import RowArchiveBtn from "@/components/RowArchiveBtn";
...
export default function Home() {
  ...
  const dbData = [
    "death note", "bleach", "hunter x hunter", "balance unlimited", "tokyo ghoul", "dr stone", "demon slayer",
    "vinland saga", "cyberpunk edgerunners", "mandelin - site and reference",  "Challenges", "anime list", "aot", "bank", "knapsack", "kangol", "gon"
  ];
        ...
        <ul className={`h-full w-full pb-10 py-1 space-y-1`}>
          {
            dbData.map((eachData, key):any => <li key={key} className="w-full">
              <RowArchiveBtn data={{id: key, title: eachData}}/>
            </li>)
          }
        </ul>
